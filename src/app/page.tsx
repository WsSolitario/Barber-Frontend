...
export default async function Home() {
  ...
  const promotions = landing?.promotions ?? [];
  ...
  const visitSteps = [
    ["01", "ELIGE TU SERVICIO", "Revisa el menú, elige lo que necesitas y conoce la duración antes de confirmar."],
    ["02", "ENCUENTRA TU MOMENTO", "Consulta disponibilidad real y elige al profesional y horario que mejor encajen contigo."],
    ["03", "LLEGA SIN PRISA", "Tu cita queda registrada para que el tiempo en el estudio sea para ti y tu estilo."],
  ];
  const questions = [
    ["¿NECESITO SABER QUÉ CORTE QUIERO?", "No. Puedes llegar con una referencia o explicarnos el resultado que buscas; la conversación forma parte de la cita."],
    ["¿PUEDO ELEGIR BARBERO?", "Sí. Cuando haya más de un profesional disponible, podrás ver tus opciones durante la reserva."],
    ["¿CÓMO CONSULTO LOS HORARIOS?", "Abre la reserva para ver los horarios disponibles para el servicio, la sucursal y el profesional seleccionados."],
    ["¿TENGO UNA DUDA ANTES DE RESERVAR?", "Escríbenos por WhatsApp. Te ayudamos a elegir el servicio adecuado antes de que apartes tu horario."],
  ];

  return (
    <main className="overflow-hidden bg-[#f4f3ed] pb-20 text-[#172b34] md:pb-0">
      ...
      <section id="proceso" className="bg-[#172b34] px-5 py-20 text-[#f4f3ed] sm:px-8 lg:py-28"><div className="mx-auto max-w-7xl"><div className="grid gap-8 border-b border-[#5d7279] pb-10 lg:grid-cols-[1fr_.7fr]"><div><p className="font-[family-name:var(--font-bebas-neue)] text-2xl tracking-[.12em] text-[#d6f22a]">TU CITA, CLARA</p><h2 className="mt-4 max-w-2xl font-[family-name:var(--font-bebas-neue)] text-6xl leading-[.85] sm:text-7xl">MENOS VUELTAS. MÁS TIEMPO PARA TI.</h2></div><p className="self-end leading-7 text-[#c8d2d2]">Reservar no debería ser otra tarea pendiente. Aquí puedes conocer el servicio, ver la disponibilidad y dejar tu turno listo en pocos pasos.</p></div><div className="grid md:grid-cols-3">{visitSteps.map(([number, title, description]) => <article className="border-b border-[#5d7279] py-8 md:border-r md:px-8 md:first:pl-0" key={number}><p className="font-[family-name:var(--font-bebas-neue)] text-4xl text-[#d6f22a]">{number}</p><h3 className="mt-12 text-xl font-extrabold">{title}</h3><p className="mt-3 max-w-xs leading-7 text-[#c8d2d2]">{description}</p></article>)}</div><Link className="mt-10 inline-block border-b-2 border-[#d6f22a] py-3 text-sm font-extrabold" href="/booking">Ver disponibilidad ahora</Link></div></section>

      {promotions.length > 0 && <section className="bg-[#d6f22a] px-5 py-14 text-[#10242c] sm:px-8"><div className="mx-auto max-w-7xl"><p className="font-[family-name:var(--font-bebas-neue)] text-2xl tracking-[.12em]">EN EL ESTUDIO</p><div className="mt-6 grid gap-6 lg:grid-cols-3">{promotions.map((promotion) => <article className="border-t-2 border-[#10242c] pt-5" key={promotion.title}><h2 className="font-[family-name:var(--font-bebas-neue)] text-4xl leading-none">{promotion.title}</h2>{promotion.description && <p className="mt-3 max-w-sm leading-7">{promotion.description}</p>}</article>)}</div></div></section>}
      ...
      <section id="preguntas" className="mx-auto max-w-7xl px-5 py-20 sm:px-8 lg:py-28"><div className="grid gap-12 lg:grid-cols-[.72fr_1.28fr]"><div><p className="font-[family-name:var(--font-bebas-neue)] text-2xl tracking-[.12em] text-[#5f747a]">ANTES DE VENIR</p><h2 className="mt-4 max-w-sm font-[family-name:var(--font-bebas-neue)] text-6xl leading-[.88]">BUENAS PREGUNTAS. MEJORES CITAS.</h2><p className="mt-6 max-w-sm leading-7 text-[#486068]">Queremos que llegues con claridad. Si no encuentras lo que buscas, estamos a un mensaje de distancia.</p><a className="mt-8 inline-block border-b-2 border-[#d6f22a] py-3 text-sm font-extrabold" href={`https://wa.me/${contact?.whatsapp ?? "5550000000"}`}>Hablar por WhatsApp</a></div><div className="border-t-2 border-[#172b34]">{questions.map(([question, answer]) => <details className="group border-b border-[#aebabc] py-6" key={question}><summary className="flex cursor-pointer list-none items-center justify-between gap-6 text-lg font-extrabold"><span>{question}</span><span className="font-[family-name:var(--font-bebas-neue)] text-3xl text-[#5f747a] transition group-open:rotate-45">+</span></summary><p className="max-w-2xl pt-4 leading-7 text-[#486068]">{answer}</p></details>)}</div></div></section>
      ...
      <div className="fixed inset-x-0 bottom-0 z-20 border-t border-[#5d7279] bg-[#10242c] p-3 md:hidden"><Link className="block bg-[#d6f22a] px-5 py-4 text-center text-sm font-extrabold text-[#10242c]" href="/booking">RESERVAR MI HORARIO</Link></div>
    </main>
  );
}
