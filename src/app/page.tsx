    ...
    <main className="overflow-hidden bg-[#f4f3ed] text-[#172b34]">
      <section className="bg-[#10242c] text-[#f4f3ed]">
        <nav className="mx-auto flex max-w-7xl items-center justify-between px-5 py-5 sm:px-8">
          ...
          <div className="hidden items-center gap-7 text-sm font-bold md:flex"><a className="transition hover:text-[#d6f22a]" href="#oficio">Oficio</a><a className="transition hover:text-[#d6f22a]" href="#trabajo">Trabajo</a><a className="transition hover:text-[#d6f22a]" href="#visita">Visita</a></div>
      ...
      <section id="oficio" className="mx-auto max-w-7xl px-5 py-20 sm:px-8 lg:py-28"><div className="grid gap-12 lg:grid-cols-[.85fr_1.15fr]"><div><p className="font-[family-name:var(--font-bebas-neue)] text-2xl tracking-[.12em] text-[#5f747a]">EL OFICIO</p><h2 className="mt-4 max-w-md font-[family-name:var(--font-bebas-neue)] text-6xl leading-[.88]">TODO EMPIEZA CON ESCUCHAR.</h2><p className="mt-6 max-w-md leading-7 text-[#486068]">Elige el servicio que necesitas. Durante la reserva podras seleccionar al profesional y el horario disponible que mejor te funcione.</p></div><div className="border-t-2 border-[#172b34]">{[["Corte clasico", "Forma, estructura y un acabado limpio para tu dia a dia."], ["Barba y perfilado", "Lineas cuidadas para que tu barba acompañe tu corte."], ["Experiencia completa", "Un servicio sin carreras, desde el lavado hasta el styling."]].map(([title, description], index) => <article className="grid gap-5 border-b border-[#aebabc] py-7 sm:grid-cols-[4rem_1fr_auto] sm:items-start" key={title}><p className="font-[family-name:var(--font-bebas-neue)] text-3xl text-[#5f747a]">0{index + 1}</p><div><h3 className="text-xl font-extrabold tracking-tight">{title}</h3><p className="mt-2 max-w-md leading-7 text-[#486068]">{description}</p></div><Link className="w-fit border-b-2 border-[#d6f22a] py-3 text-sm font-extrabold" href="/booking">Reservar</Link></article>)}</div></div></section>
